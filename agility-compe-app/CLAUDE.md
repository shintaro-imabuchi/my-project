# プロジェクト概要
<!-- アプリの目的・概要を記載 -->
- 犬のアジリティー競技会向けに、参加者が利用する参加登録、管理者が利用する参加者・犬一覧、競技種目別出走表・成績表作成などを行うアプリ
- 参加登録のエントリーポイントと管理者用のエントリーポイントを分ける
- 参加者が利用する参加登録アプリはスマートフォンやタブレットからの利用を想定してレスポンシブ対応とし、streamlitのサイドバーは基本的に使用しない
- 管理者用アプリはPCでの利用が主になるので、操作性を高めるサイドバーの利用は問題ない
...
├── README.md
├── requirements.txt
├── app_entry.py                  # 参加登録エントリーポイント
├── app_admin.py                  # 管理者エントリーポイント
